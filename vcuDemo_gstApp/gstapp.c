...
#define SINKBUSID "a0080000.v_mix"
...
create_display_count_bin()
{
  ...
  g_object_set (sink, "plane-id", 37, "bus-id", SINKBUSID, NULL);
...
create_decode_bin (guint idx, int detection_type, gchar *location, guint plane_id, guint x_pos, guint y_pos)
{
...
#endif
  vconvert = gst_element_factory_make("xlnxscale", NULL);
  ...
  g_object_set (vconvert, "need-tdm", TRUE, NULL);
...
main (int argc, char **argv)
{
  ...
  bin = create_decode_bin (0, ML_FACE_DETECTION, url, 29, 0, 0);
  if (bin == NULL) {
  ...
  printf ("face URL - 2: %s\n", url);
  bin = create_decode_bin (1, ML_FACE_DETECTION, url, 30, ML_INPUT_WIDTH, 0);
  if (bin == NULL) {
    printf ("bin not created...\n");
	return -1;
  }
  ...
  printf ("face URL - 3: %s\n", url);
  bin = create_decode_bin (2, ML_FACE_DETECTION, url, 31, 0, ML_INPUT_HEIGHT);
  if (bin == NULL) {
    printf ("bin not created...\n");
	return -1;
  }
  ...
  printf ("face URL - 4: %s\n", url);
  bin = create_decode_bin (3, ML_FACE_DETECTION, url, 32, ML_INPUT_WIDTH, ML_INPUT_HEIGHT);
  if (bin == NULL) {
    printf ("bin not created...\n");
	return -1;
  }
  ...
  bin = create_decode_bin (0, ML_TRAFFIC_DETECTION, url, 33, 2*ML_INPUT_WIDTH, 0);
  if (bin == NULL) {
    printf ("bin not created...\n");
	return -1;
  }
  ...
  bin = create_decode_bin (1, ML_TRAFFIC_DETECTION, url, 34, 3*ML_INPUT_WIDTH, 0);
  if (bin == NULL) {
    printf ("bin not created...\n");
	return -1;
  }
  ...
  bin = create_decode_bin (2, ML_TRAFFIC_DETECTION, url, 35, 2*ML_INPUT_WIDTH, ML_INPUT_HEIGHT);
  if (bin == NULL) {
    printf ("bin not created...\n");
	return -1;
  }
  ...
  bin = create_decode_bin (3, ML_TRAFFIC_DETECTION, url, 36, 3*ML_INPUT_WIDTH, ML_INPUT_HEIGHT);
  if (bin == NULL) {
    printf ("bin not created...\n");
	return -1;
  }
